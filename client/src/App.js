  ...
  const { data } = this.state;
   ...
   return (
     <div>
       ...
       <div style={{height: '400px', width:'900px', border:'1px solid #ccc', font:'16px/26px Georgia, Garamond, Serif', overflow:'auto'}}>
       <ul>
         <h2>{ idCount= data.length} recipes:</h2>
          {data.length <= 0
            ...
            : data.map(dat => (
                ...
                <li class="form-group" style={{ padding: "10px" }} key={data.recipeInstructions} > 
                    <span> <b> User:</b> </span> {dat.recipeUser} <br />
                  <span > <b> Category:</b> </span>{dat.recipeCategory} <br />              
                  <span > <b>Recipe ID:</b> </span> <input   id="theRecipeId"  type="number" value = {dat.recipeId}  style={{width: '30px'}} readonly   onClick ={event => this.setState({ recipeId: event.target.value })}/> <br/> <b> Recipe Name: </b> <input class="form-control"  id="theRecipeName"   value={dat.recipeName} onClick={event => this.setState({ recipeName: event.target.value })}/> <br /> <span> <b>Enter a rating:</b> </span><input  type="number" min="1" max="5" onChange={(event) => {this.setState({ recipeRating: event.target.value })} } /> <br/> <b> Illustration:</b> <img src={dat.recipePicture} width="350" height="300" style={{paddingRight: "20"}}  alt={dat.recipeName} /> <br/>
                 
                 <b>Submitted by</b><input class="form-control"  type="text" onChange={(event) => {this.setState({recipeUser: event.target.value})}} /><button id="idName" class="btn btn-success" onClick={this.setRecipeRating.bind(this)}>SUBMIT RATING</button><br /><b>(click on Recipe Id and Recipe Name fields before submitting)</b>   <br />             
                  <span > <b> Instructions:</b> </span><textarea class="form-control" rows="8" cols="35">{dat.recipeInstructions}</textarea>              
...
const appHeading=<div><head>    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u"
crossorigin="anonymous" /> </head><h1>Virtual Chef Recipes</h1> </div>;
const Home = () => (
  <div>
...
class SearchRecipes extends React.Component {
  ...
  findRecipeByName = () => {
    ...
    render () {
     ...
     <table>
     <tr>
         <td>SEARCH BY USER NAME:</td><td> <div class="form-group"><input class="form-control" type="text"  onChange={event => this.setState({ RecipeUser: event.target.value })} /></div></td><td><button class="btn btn-default" onClick={this.findRecipeByUser.bind(this)}>SEARCH</button></td>
         </tr>  
     <tr>
         <td>SEARCH BY CATEGORY:</td><td> <div class="form-group"><select class="form-control" onChange={event => this.setState({ RecipeCategory: event.target.value })}><option value="Main Dish">Main Dish</option><option value="Side Dish">Side Dish</option><option value="Salad">Salad</option><option value="Dessert">Dessert</option></select></div></td><td><button class="btn btn-default" onClick={this.findRecipeByCategory.bind(this)}>SEARCH</button></td>
         </tr>     
       <tr>
         <td>SEARCH BY RECIPE NAME:</td><td> <div class="form-group"><input class="form-control" type="text"  onChange={event => this.setState({ RecipeName: event.target.value })} /></div></td><td><button class="btn btn-default" onClick={this.findRecipeByName.bind(this)}>SEARCH</button></td>
         </tr>
         <tr>
         <td>SEARCH BY INSTRUCTIONS:</td><td> <div class="form-group"><input class="form-control"  type="text"  onChange={event => this.setState({ RecipeInstructions: event.target.value })} /></div></td><td><button class="btn btn-default" onClick={this.findRecipeByInstructions.bind(this)}>SEARCH</button></td>
         </tr>
       </table>
       <ul>
       {data.length <= 0
...
class ViewRatings extends React.Component {
 ...
 return (
   <div>
     <table>
       <tr>
   <td>SEARCH BY ID: <div class="form-group"><input class="form-control"   type="number" onChange={event => this.setState({ recipeID: event.target.value })} value={this.state.recipeID} /></div></td><td><button class="btn btn-default"  onClick={this.getRatings.bind(this)}>SEARCH BY RECIPE ID</button></td>
      </tr>
      <tr>
  <td> SEARCH BY RECIPE NAME: <div class="form-group"><input class="form-control"  type="text"  onChange={event => this.setState({ recipeName: event.target.value })} value={this.state.recipeName} /></div></td><td><button  class="btn btn-default" onClick={this.getRatingsByRecipeName.bind(this)}>SEARCH BY RECIPE NAME</button></td>
    </tr>
    <tr>
   <td>SEARCH BY RECIPE USER:<div class="form-group"> <input class="form-control"  type="text"  onChange={event => this.setState({ recipeUser: event.target.value })} value={this.state.recipeUser} /></div></td><td><button class="btn btn-default"  onClick={this.getRatingsByUser.bind(this)}>SEARCH BY RECIPE USER</button></td>
    </tr>
   </table>
   {data.length <= 0
   ? '' :
...
setRecipeRating () {
  ...
  <form>
        ...
        <table>
      <tr>
      <td>  UserName: </td> <td><div class="form-group"><input class="form-control" type="text" onChange={event => this.setState({ RecipeUserName: event.target.value })} id="userName" value={this.state.RecipeUserName}  /></div> </td>
      </tr>
      <tr>
      <td>  Category: </td> <td><select class="form-control" onChange={event => this.setState({ RecipeCategory: event.target.value })}><option value="Main Dish">Main Dish</option><option value="Side Dish">Side Dish</option><option value="Salad">Salad</option><option value="Dessert">Dessert</option></select> </td>
      </tr>    
      <tr>
      <td>  Recipe description: </td> <td><div class="form-group"><input class="form-control" type="text"  onChange={event => this.setState({ RecipeName: event.target.value })} id="recipeDescription" value={this.state.RecipeName} size="50" /></div></td>
      </tr>
      <tr>
      <td>  Recipe instructions: </td> <td><div class="form-group"> <textarea class="form-control" id="recipeInstructions" onChange={event => this.setState({ RecipeDirections: event.target.value })} value={this.state.RecipeDirections} rows="4" cols="75" ></textarea> </div></td> 
      </tr>
      <tr>
      <td>  Recipe picture (URL):</td> <td><div class="form-group"><input  class="form-control"  type="text" id="recipePicture" size="80" onChange={event => this.setState({ RecipePic: event.target.value })} value={this.state.RecipePic} /></div></td>
      </tr>
      <tr>
      <td>&nbsp; </td> <td><button class="btn btn-primary"  onClick={this.postRecipe.bind(this)}>SUBMIT RECIPE</button></td>
      </tr> 
      <tr>
