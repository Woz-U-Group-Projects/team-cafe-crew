...
class RecipeData extends React.Component {
 ...
 postRecipe  ()  {
    ...
    this.setState( {RecipeObject: {userName: this.state.RecipeUserName, recipeTitle: this.state.RecipeName, recipePicture: this.state.RecipePic, recipeInstructions: this.state.RecipeDirections}}); 
  ...
  listItems() {
      ...
      <ul>
        {
         items.map((val, index) => {
            return (
              <li key={index}>
              <b> { val.recipeTitle}</b> - {val.userName } <br /> <img src={val.recipePicture} width="120" height="140" style={{paddingRight: "20"}} /><textarea rows="9" cols="30">{val.recipeInstructions}</textarea>
              </li>
            );
          })
        }
   ...
   render () {
    return (
      ...
      <tr>
      <td>  Recipe picture (URL):</td> <td><input type="text" id="recipePicure" size="80" onChange={event => this.setState({ RecipePic: event.target.value })} value={this.state.RecipePic} /></td>
      </tr>
      <tr>
      <td>&nbsp; </td> <td><button onClick={this.postRecipe.bind(this)}>SUBMIT RECIPE </button></td>
      </tr> 
      <tr>
      <td>&nbsp; </td> <td><button onClick={this.updateRecipeList.bind(this)}>UPDATE RECIPE LIST </button></td>
      </tr>            
        </table>
       {this.listItems()}
        </div>
