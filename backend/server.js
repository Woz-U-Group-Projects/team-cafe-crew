...
//const dbRoute = "mongodb://skmeyer2019:Reactive_19@ds245234.mlab.com:45234/recipes";
//const dbRatings = "mongodb://skmeyer2019:SDale2019@ds145304.mlab.com:45304/reciperatings";
//const dbRoute = "mongodb://rcanada44:MuayThai1974$$@ds135724.mlab.com:35724/virtualcafe?authSource=admin";
...
router.get("/viewAllRecipes", (req, res) => {
 ...
 router.get("/viewRatingsByUser/recipeUser/:recipeUser", (req, res) => {
  RatingsData.find({recipeUser: new RegExp(req.params.recipeUser,"i")},  (err,data) => { 
// RatingsData.find({recipeName: req.params.recipeName},  (err,data) => { 
   if (err) return res.json({ success: false, error: err });
   console.log(data);
   res.json({ success: true, data: data });
 ...
 router.get("/findRecipeByUser/recipeUser/:recipeUser",  (req, res) => {
  Data.find({recipeUser: new RegExp(req.params.recipeUser,"i")},  (err,data) => { 
    if (err) return res.json({ success: false, error: err });
    console.log(data);
    res.json({ success: true, data: data });   
  });
}); 
router.get("/findRecipeByName/recipeName/:recipeName",  (req, res) => {
  Data.find({recipeName: new RegExp(req.params.recipeName,"i")},  (err,data) => { 
    if (err) return res.json({ success: false, error: err });
    console.log(data);
    res.json({ success: true, data: data });   
  });
...
router.get("/findRecipeByCategory/recipeCategory/:recipeCategory",  (req, res) => {
  Data.find({recipeCategory: new RegExp(req.params.recipeCategory,"i")},  (err,data) => { 
    if (err) return res.json({ success: false, error: err });
    console.log(data);
    res.json({ success: true, data: data });   
  });
...
router.get("/findRecipeByInstructions/recipeInstructions/:recipeInstructions",  (req, res) => {
  Data.find({recipeInstructions: new RegExp(req.params.recipeInstructions,"i")},  (err,data) => { 
    if (err) return res.json({ success: false, error: err });
    console.log(data);
    res.json({ success: true, data: data });   
  });
